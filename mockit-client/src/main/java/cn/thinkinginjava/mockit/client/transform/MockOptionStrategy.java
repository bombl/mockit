 ...
 */
public class MockOptionStrategy implements OptionStrategy {
    ...
    public void execute(String text) throws Exception {
        MockData mockData = GsonUtil.fromJson(text, MockData.class);
        MockTransformerManager.transformer(
                new ResultMockClassFileTransformer(mockData.getClassName(),
                        mockData.getMethodName(), mockData.getMockValue()),
                Class.forName(mockData.getClassName()));
    }
}